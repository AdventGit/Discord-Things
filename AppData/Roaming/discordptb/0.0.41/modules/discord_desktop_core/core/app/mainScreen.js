...
var INITIAL_PATH = extractPathFromArgs(process.argv.slice(1), '/activity');
...
var DEFAULT_HEIGHT = 1050;
var ACCOUNT_GREY = 'rgba(0, 0, 0, 0)';
...
function init() {
  ...
  _electron.app.on('open-url', function (event, openURL) {
    ...
    if (parsedURL.protocol === 'discord:') {
      if (mainWindow == null) {
        // TODO: This won't actually do anything, because it gets overridden where appPath gets used
        // INITIAL_PATH = parsedURL.path;
      } else {
...
function handleSingleInstance(args) {
  if (mainWindow != null) {
    // TODO: this used to rely on module-scoped appPath (now INITIAL_PATH), but
    // changing it at module scope shouldn't affect anything.
